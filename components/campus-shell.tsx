...
import {Bell,BookOpen,ChevronRight,Compass,Home,LayoutGrid,Menu,MessageCircle,Search,Sparkles,Users,X,Zap} from 'lucide-react';
...
const nav=[['/','01 / OVERVIEW',Home],['/discover','02 / DISCOVER',Compass],['/clubs','03 / CLUBS',Users],['/resources','04 / RESOURCES',BookOpen],['/community','05 / COMMUNITY',MessageCircle]] as const;
...
export default function CampusShell({children}:{children:React.ReactNode}){
 const pathname=usePathname(); const [mobile,setMobile]=useState(false); const [palette,setPalette]=useState(false); const [query,setQuery]=useState(''); const [selected,setSelected]=useState(0); const [unread,setUnread]=useState(0);
 ...
 return <div className={styles.app}>
  <aside className={`${styles.sidebar} ${mobile?styles.mobileOpen:''}`}>
   ...
   <nav className={styles.nav} aria-label="Primary navigation">{nav.map(([href,label,Icon])=>{const active=isActive(href);return <Link key={href} href={href} onClick={()=>setMobile(false)} className={active?styles.active:''} aria-current={active?'page':undefined}><Icon size={13}/><span>{label}</span>{label.includes('DISCOVER')&&<b>05</b>}</Link>})}<Link href="/profile" onClick={()=>setMobile(false)} className={isActive('/profile')?styles.active:''} aria-current={isActive('/profile')?'page':undefined}><LayoutGrid size={13}/><span>06 / PROFILE</span></Link></nav>
   <button className={styles.sideCard} onClick={openCopilot}><div className={styles.sideOrb}><Sparkles size={15}/></div><div><b>Campus Copilot</b><p>Ask about deadlines, clubs or this week.</p></div><span aria-hidden="true">↗</span></button>
