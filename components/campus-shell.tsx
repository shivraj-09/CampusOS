...
export default function CampusShell({children}:{children:React.ReactNode}){
 ...
 return <div className={`${styles.app} ${dark?styles.dark:''}`}>
  ...
  <main className={styles.main}>
   <header className={styles.topbar}><button className={styles.menu} onClick={()=>setMobile(true)}><Menu size={20}/></button><button className={styles.search} onClick={()=>{setPalette(true);setQuery('');setSelected(0)}}><Search size={16}/><span>Search campus...</span><kbd>⌘ K</kbd></button><div className={styles.topActions}><button title="Toggle theme" onClick={()=>setDark(v=>!v)}><Sun size={17}/></button><Link href="/notifications" title="Notifications" aria-label={`Notifications${unread?`, ${unread} unread`:''}`}><Bell size={17}/>{unread>0&&<i/>}</Link><Link className={styles.profileDot} href="/profile">S</Link></div></header>
   {children}
  </main>
  <CampusCopilot/>
