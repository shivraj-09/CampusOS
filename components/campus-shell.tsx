...
import {Bell,BookOpen,CalendarDays,ChevronRight,Command,Compass,Home,LayoutGrid,Menu,MessageCircle,Search,Sparkles,Sun,Users,X,Zap} from 'lucide-react';
...
export default function CampusShell({children}:{children:React.ReactNode}){
 ...
 const results=useMemo(()=>{const q=query.trim().toLowerCase();if(!q)return[];return [...opportunities.map(x=>({title:x.title,meta:x.type,href:'/discover'}),...clubs.map(x=>({title:x.name,meta:'Club',href:'/clubs'})),...resources.map(x=>({title:x.title,meta:x.type,href:'/resources'})),...posts.map(x=>({title:x.title,meta:'Community',href:'/community'}))].filter(x=>(x.title+' '+x.meta).toLowerCase().includes(q)).slice(0,6)},[query]);
