...
const readPosts=():Post[]=>{try{return JSON.parse(localStorage.getItem(postKey)||'[]')}catch{return[]}};
...
export default function CommunityFeed() {
  ...
  return <section className={styles.wrap}><div className={styles.header}><div><span>LIVE CAMPUS FEED</span><h1>People are building.</h1><p>Ask, share, collaborate, and find your next teammate.</p></div><button onClick={() => setOpen(true)}><Plus size={16}/> Start a discussion</button></div><div className={styles.feed}>{allPosts.map(post => <PostCard key={post.id} post={post}/>)}</div>{open&&<div className={styles.overlay}><form className={styles.modal} onSubmit={submit}><button type="button" className={styles.close} onClick={() => setOpen(false)}><X size={18}/></button><span>NEW DISCUSSION</span><h2>What are you thinking?</h2><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Give your post a clear title" autoFocus/><textarea value={body} onChange={e=>setBody(e.target.value)} placeholder="Share context, ask a question, or show what you built..." rows={6}/><button className={styles.publish} type="submit"><Send size={15}/> Publish to campus</button></form></div>}</section>;
}
